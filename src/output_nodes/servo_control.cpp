...
void ServoMotor::move(int time, int angle) {
...
void ServoMotor::setHigh() {
    if (gpiod_line_set_value(line, 1) < 0) {
        throw std::runtime_error("Failed to set GPIO line high");
    }
}

void ServoMotor::setLow() {
    if (gpiod_line_set_value(line, 0) < 0) {
        throw std::runtime_error("Failed to set GPIO line low");
    }
}
